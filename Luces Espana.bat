@echo off
title Luces Espana - lanzador
chcp 65001 >nul
cd /d "%~dp0"

echo ============================================
echo   LUCES - Himno de Espana (bandera)
echo ============================================
echo.
...
REM --- 2) Proyecto de arranque = himno_espana ---
set LUCES_PROJECT=himno_espana

REM --- 3) Arrancar el backend headless en su propia ventana ---
echo Arrancando backend web (proyecto: %LUCES_PROJECT%)...
start "Luces backend (Espana)" "%cd%\venv311\Scripts\python.exe" -m server.main
...
if errorlevel 1 (
  if %tries% geq 30 (
    echo.
    echo [ERROR] El servidor no arranco en 30s. Revisa la ventana "Luces backend (Espana)".
    pause
    exit /b 1
  )
  goto wait
...
echo.
echo La app esta corriendo en http://localhost:8000 con el Himno de Espana.
echo Para detenerla: cierra la ventana "Luces backend (Espana)" o usa "Cerrar Luces.bat".
