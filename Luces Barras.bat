@echo off
title Luces Barras - lanzador
chcp 65001 >nul
cd /d "%~dp0"

echo ============================================
echo   LUCES - El Taser (barras LED, 0-1:30)
echo ============================================
echo.
...
REM --- 2) Proyecto de arranque = el_taser_barras ---
set LUCES_PROJECT=el_taser_barras

REM --- 3) Arrancar el backend headless en su propia ventana ---
echo Arrancando backend web (proyecto: %LUCES_PROJECT%)...
start "Luces backend (Barras)" "%cd%\venv311\Scripts\python.exe" -m server.main
...
if errorlevel 1 (
  if %tries% geq 30 (
    echo.
    echo [ERROR] El servidor no arranco en 30s. Revisa la ventana "Luces backend (Barras)".
    pause
    exit /b 1
  )
  goto wait
...
echo.
echo La app esta corriendo en http://localhost:8000 con El Taser (barras LED).
echo Para detenerla: cierra la ventana "Luces backend (Barras)" o usa "Cerrar Luces.bat".
