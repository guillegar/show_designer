...
(Fase 8 de la auditoría): la web es la única interfaz. Todo vive en `server/` (Python) y `web/`.

```
show-designer/
│
├── src/                      ← CÓDIGO NÚCLEO (lo consume el backend web server/)
...
- El backend `server/` reutiliza SIN cambios `src/core`, `src/analysis`, `src/io`, `src/mcp`.
